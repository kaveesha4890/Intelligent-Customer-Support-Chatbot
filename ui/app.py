...
<body>
    <div class="demo-badge">&#128274; DEMO MODE &mdash; Simulated Data Only</div>
    ...
    <div class="back-link">No account yet? Switch to the
        <a href="#" onclick="showTab('signup');return false;">Sign Up</a> tab above.</div>
    ...
    function appendBotMessage(text, turnId, intent) {
        ...
        const fbHtml = showFeedback
            ? '<div class="feedback-row">'
              ...
              + '</div>'
            : '';
        row.innerHTML = '<div class="msg bot' + (isTyping?' typing':'') + '">'
                        + escHtml(text) + '</div>' + fbHtml;
        win.appendChild(row);
        win.scrollTop = win.scrollHeight;
